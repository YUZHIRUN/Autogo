...
def __code_proc(input_code: str):
    info_list = list()
    code_obj = common.file_info_clean(input_code)
    if_phrase_list = if_phrase_proc.if_phrase(code_obj)
    if_phrase_num = len(if_phrase_list)
    if_idx = 0
    task_list = common.pack_func_info(code_obj)  # Package valid information in a function
